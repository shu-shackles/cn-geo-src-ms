...
def user_setinfo(password, type, area, uid):
  con.execute(f'UPDATE users SET password = \'{password}\', type = {type}, AREA = \'{area}\'  WHERE uid = {uid}')
  sql_result = con.execute(f'select * from users where area=\'{area}\' and password = \'{password}\' and uid = {uid} and type = {type}')
  if sql_result.all():
    return True
  else:
    return False
